...
class SquareRoutine : public rclcpp::Node
{
  ...
  private:
	...
	void timer_callback()
	{
	    ...
	    else if (fabs(wrap_angle(th_now - th_init)) < fabs(th_aim))
	    {
	        msg.linear.x = 0; 
	        msg.angular.z = (th_aim > 0) ? th_vel : -th_vel; // Adjust direction based on th_aim
	        publisher_->publish(msg);        
	    }        
	    // If done step, stop
	    else
		...
		if (last_state_complete == 1)
		{
			switch(count_) 
			{
			  case 0:
			    ...
			    break;
			  case 1:
			    turn_angle(M_PI/2.02);  //turn to avoid wall
			    break;
			  case 2:
			    move_distance(0.45);  //second movement for the robot
			    break;
			  case 3:
			    turn_angle(-M_PI/2.02); //turn the robot again
			    break;
			  case 4:
			  ...
			  case 6:
			    ...
			    break;			    
			  case 7:
			    turn_angle(M_PI/2.02);  //turn towards cargo
			    break;  
              case 8:
			  ...
			  case 9: //start truning around
				...
				break;			  
			  case 10:
			    turn_angle(M_PI/2.02); //robot did 180
				break;
			  case 11:
			    move_distance(1.2);
				break;
	...
	double x_vel = 0.2, th_vel = 0.5; // Increased turn velocity for testing
